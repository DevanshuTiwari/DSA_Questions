...
public class ConstructBinaryTreeFromPreAndInOrder {
    ...
    private static TreeNode build(int[] preorder, int preStart, int preEnd, int[] inorder, int inStart, int inEnd, Map<Integer, Integer> inMap) {
        ...
        root.left = build(preorder, preStart + 1, preStart + numsLeft, inorder, inStart, inRootIndex - 1, inMap);
        root.right = build(preorder, preStart + numsLeft + 1, preEnd, inorder, inRootIndex + 1, inEnd, inMap);
